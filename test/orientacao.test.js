...
// celular. Percorre todas as telas e estados num navegador de verdade e falha
// se algum elemento com texto visível tiver rotação diferente de 0° (somando
// o giro de todos os ancestrais). Sem Chrome/Edge na máquina, a parte do
...
describe('orientação: checagem estática', () => {
  test('nenhum rotate(180deg) no CSS nem no HTML', () => {
    assert.doesNotMatch(css, /rotate\(\s*180deg\s*\)/);
...
// Giro de cada elemento com texto visível, somando os ancestrais.
const GIRADOS = `(() => {
  const giro = (el) => {
    let m = new DOMMatrix();
  ...
  for (const el of document.querySelectorAll('body *')) {
    if (!['SCRIPT', 'STYLE', 'TEMPLATE'].includes(el.tagName)
      ...
      const g = giro(el);
      if (Math.abs(g) > 0.5) fora.push(el.textContent.trim().slice(0, 24) + ' (' + g.toFixed(1) + '°)');
    }
  }
  return fora;
})()`;
...
describe('orientação: auditoria no navegador (todas as telas e estados)', { skip: executavel ? false : 'sem Chrome/Edge (defina NAVEGADOR=...)', timeout: 120000 }, () => {
  ...
  test('batalha: Rolar, Mira, Batalha, contra o Rato e fim', async () => {
    ...
    for (let i = 0; i < 12; i++) {
      if (await p.js(`document.querySelector('#btn-lutar .lutar-texto').textContent === 'VER FIM'`)) break;
      if (i) await p.tocar('#metade-0 .palco-luta');
      await rodada('DEFESA', 'ATAQUE');
      await esperarFim();
    }
    await p.tocar('#btn-lutar');
    assert.equal(await telaAtual(), 'tela-fim');
    await conferir('fim');
  });
